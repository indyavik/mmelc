(function (A) {
    A.extend(A.fn, { pstrength: function (B) {
        var B = A.extend({ verdects: ["Too Weak", "Weak", "Medium", "Strong", "Very strong"],
            colors: ["#f00", "#c06", "#f60", "#3c0", "#3f0"], scores: [10,15,20, 40],
                    ...
                    if (nPerc <= C.scores[0]) {
                        ...
                        if (nPerc > C.scores[0] && nPerc <= C.scores[1]) {
                            ...
                            if (nPerc > C.scores[1] && nPerc <= C.scores[2]) {
                                ...
                                if (nPerc > C.scores[2] && nPerc <= C.scores[3]) { strColor = C.colors[3]; strText = C.verdects[3]; A(B).css({ width: "100px" }) } else { strColor = C.colors[4]; strText = C.verdects[4]; A(B).css({ width: "145px" }) }
                            }
                        }
                    }
                }
            } A(B).css({ backgroundColor: strColor }); A(E).html("<span style='color: " + strColor + ";'>" + strText + "</span>")
        }, 
        //Vishala : Modified the computation logic below to suit our requirment.
        checkPassword: function (C, B) {
            var F = 0; var E = B.verdects[0];
            if (C.length < B.minchar) {
                ...
                if (C.match(/[a-z]/)) {
                    F = (F + 5)
                } if (C.match(/[A-Z]/) || (!B.uppercase)) {
                    F = (F + 9)
                }
                if (C.match(/\d+/)) {
                    F = (F + 5)
                }
                if (C.match(/.[!,@,#,$,%,^,&,*,?,_,~]/) || (!B.one_special_char)) {
                    F = (F + 9)
                }
                /*if (C.match(/(.*[!,@,#,$,%,^,&,*,?,_,~].*[!,@,#,$,%,^,&,*,?,_,~])/) || (!B.two_special_char)) {
                F = (F + 7)
