...
(function (PxlViewer) {
    ...
    $.extend(PxlViewer.Base.prototype, {
		...
		copyCachedImageToBackgroundCanvas : function()
        {
		...
		copyCurrentImageToBackgroundCanvas: function()
        {
            var backgroundCanvasId = "backgroundCanvas";
            var backgroundCanvas = $('#' + backgroundCanvasId);
            var sourceCanvas = this.viewer.drawer.canvas;
            ...
            {
                //console.log("using ZCacheAutoPreCachingIndex " + pxl.viewer.drawer.ZCacheAutoPreCachingIndex);
            ...
            // DRAW CACHED TEXT IN BOTTOM CORNER
            /*offscreenContext.fillStyle = "white";
            offscreenContext.font = "bold 24px Arial";
            offscreenContext.lineWidth = 1;
            offscreenContext.strokeStyle = 'black';
            offscreenContext.strokeText("CACHED " + cachedCanvasID, 4, destinationCanvas.height - 4);
            offscreenContext.fillText("CACHED " + cachedCanvasID, 4, destinationCanvas.height - 4);*/

            //console.log("Creating cached canvas for " + cachedCanvasID);
            pxl.viewer.drawer.ZStackCanvasCache[cachedCanvasID] = offscreenContext;

            //pxl.debugCanvasContextFill(offscreenContext, "red");
            //$(backgroundCanvas).css("z-index", "-1");

            pxl.drawPreCachedBarsInAllCachedImages();
            ...
            if(pxl.viewer.drawer.immediateRender == true)
            {
                ...
                if(pxl.viewer.drawer.isZCacheAutoPreCaching == false)
                {
                    //console.log("draw offscreenCanvas to destinationCtx - blue");
//                    destinationCtx.drawImage(offscreenCanvas, 0, 0);
                    destinationCtx.drawImage(pxl.viewer.drawer.ZStackCanvasCache[pxl.viewer.drawer.sliderValue].canvas, 0, 0);
                    //pxl.debugCanvasContextFill(destinationCtx, "blue");
                }
                else
                {
                    //console.log("draw offscreenCanvas to destinationCtx - white");
                    //                    destinationCtx.drawImage(offscreenCanvas, 0, 0);
                    destinationCtx.drawImage(pxl.viewer.drawer.ZStackCanvasCache[pxl.viewer.drawer.sliderValue].canvas, 0, 0);
                    //pxl.debugCanvasContextFill(destinationCtx, "white");
                }
                //console.log("bring background canvas to layer 1");
                //$(backgroundCanvas).css("z-index", "1");
                //$(backgroundCanvas).css("display", "inline");
            }
            else
            {
                //console.log("draw black rect in corner, no immediateRender");
                destinationCtx.drawImage(offscreenCanvas, 0, 0);
                //pxl.debugCanvasContextFill(destinationCtx, "black");
            }

            
            //$(backgroundCanvas).css("left", "350px");

            /*// try to precache the image beside this one
            var nextSlideIndexToCache = pxl.getfocus() - 1;
            console.log("Trying to pre-cache " + nextSlideIndexToCache);
            pxl.viewer.drawer.isAutoPreCaching = true;
            pxl.loadZStacks(pxlViewerBase.Images[pxl.currentImage], pxl.getfocus(), nextSlideIndexToCache);*/

//            console.log("Zoom = " + pxl.viewer.viewport.getZoom()); 
//            console.log("MaxZoom = " + pxl.viewer.viewport.getMaxZoom());

            var bZoomLevelAutoCaches = pxl.viewer.viewport.getZoom() > ( pxl.viewer.viewport.getMaxZoom() * 0.45 );
            //console.log( "bZoomLevelAutoCaches = " + bZoomLevelAutoCaches);
            if(bZoomLevelAutoCaches||(pxl.viewer.drawer.immediateRender == true))
            {
                pxl.checkForAutoPreCachingOfNextZStack();
            }
        },
        checkForAutoPreCachingOfNextZStack: function()
        {
            //return false;
            //if( pxl.viewer.drawer.isAutoPreCaching === false)
            {
                var aNextIndexToLoad = pxl.findNextUnCachedZStack();
                if(aNextIndexToLoad != null)
                {
                    //console.log("checkForAutoPreCachingOfNextZStack loading " + aNextIndexToLoad);
                    if(pxl.viewer.drawer.isZCacheAutoPreCaching == false)
                    {
                        var backgroundCanvasId = "backgroundCanvas";
                        var destinationCanvas = document.getElementById(backgroundCanvasId);

                        //console.log("XXXXX = " + pxl.viewer.drawer.ZStackCanvasCache[pxl.viewer.drawer.sliderValue]);

                        destinationCtx = destinationCanvas.getContext('2d');
                        destinationCtx.drawImage(pxl.viewer.drawer.ZStackCanvasCache[pxl.viewer.drawer.sliderValue].canvas, 0, 0);
                        pxl.viewer.drawer.isZCacheAutoPreCaching = true;

                        //console.log("SAVE MAIN TILES MATRIX FOR LAYER " + pxl.viewer.drawer.sliderValue);
                        //pxl.viewer.drawer.tilesMatrixBackup = JSON.parse(JSON.stringify(pxl.viewer.drawer.tilesMatrix));

                        var sliderMin = $('#range').noUiSlider('options').range.min[0];
                        var sliderMax = $('#range').noUiSlider('options').range.max[0];

                        pxl.viewer.drawer.tilesMatrixBackup = {};

                        for(var prop in pxl.viewer.drawer.tilesMatrix)
                        {
                            pxl.viewer.drawer.tilesMatrixBackup[prop] = pxl.viewer.drawer.tilesMatrix[prop];
                        }
                    ...
                    pxl.viewer.drawer.ZCacheAutoPreCachingIndex = aNextIndexToLoad;
                    //$("#range").val(aNextIndexToLoad);
                    pxl.loadZStacks(this.images[pxl.currentImage], aNextIndexToLoad, pxl.regions.options.activeStackId);//100);//aNextIndexToLoad);//pxl.regions.options.activeStackId);

/*                    var backgroundCanvasId = "backgroundCanvas" + pxl.PxlIndex;
                    var backgroundCanvas = $('#' + backgroundCanvasId);
                    $(backgroundCanvas).css("z-index", "1");*/

                    //pxl.ZStacks.HookupEvents.updateZstacks();
  //                  PxlViewer.ZStacks.HookupEvents.updateZstacks();
                }
            }
        },
        findNextUnCachedZStack: function()
        {
            var sliderValue = pxl.viewer.drawer.sliderValue;
            var sliderMin = $('#range').noUiSlider('options').range.min[0];
            var sliderMax = $('#range').noUiSlider('options').range.max[0];

            //alternate between one above and one below, until max and min range have been checked
            //if all are filled, then return null
            //otherwise return id
            //pxl.viewer.drawer.ZStackCanvasCache[cachedCanvasID];

            // check value above first
            var bCheckUp = false;
            var checkOffset = 1;
            var bLowestCheck = false;
            var bHighestCheck = false;
            var sliderValueToCheck;

//debugger;
            while((bLowestCheck == false) || (bHighestCheck == false))
            {
                bCheckUp = !bCheckUp;
                if(bCheckUp)
                {
                    sliderValueToCheck = sliderValue + checkOffset;
                    if(sliderValueToCheck > sliderMax)
                    {
                        bHighestCheck = true;
                        continue;
                    }
                ...
                {
                    sliderValueToCheck = sliderValue - checkOffset;
                    // increase offset for next check, should one be required
                    checkOffset += 1;
                    if(sliderValueToCheck < sliderMin)
                    {
                        bLowestCheck = true;
                        continue;
                    }
                ...
                if(pxl.viewer.drawer.ZStackCanvasCache[sliderValueToCheck] == undefined)
                {
                    //console.log("No cached image found at index " + sliderValueToCheck);
                    return sliderValueToCheck;
                }
            }

            if(pxl.viewer.drawer.isZCacheAutoPreCaching)
            {
                pxl.viewer.drawer.isZCacheAutoPreCaching = false;
                pxl.loadZStacks(this.images[pxl.currentImage], pxl.viewer.drawer.ZCacheAutoPreCachingIndex, pxl.viewer.drawer.ZCacheAutoPreCachingIndex);
...
//                console.log("set z-index to -1 and set display to none");
                        ...
                        if (pxl.internalProperties.panning.enabled) {
                            ...
                            if (!event.preventDefaultAction) {
                                ...
                                case 104: // numpad 8
                                    ...
                                    pxl.viewer.viewport.zoomTo(pxl.viewer.viewport.imageToViewportZoom(100 / currMag), center, false);
                                    return false;
        ...
        */
        resetBackgroundCanvas: function () {
            return; // do we ever need to clear the canvas, better to keep it hidden and only show when it has a valid image?
            if (this.viewer.ZstackCallCount != null && this.viewer.ZstackCallCount != undefined) {
                this.viewer.ZstackCallCount = 1;
            }
            var canvas = document.getElementById('backgroundCanvas');
        ...
        displayViewerError: function(errorinfo) {
            ...
            var html = Mustache.render("<div id='ImageServerErrorOccured'>Slide could not be displayed. Please click on the help icon (<img src='content/images/help_pxl.png'  style='width:30px'>) in the left panel for instructions on solving this error.<br />Error code: {{{errorNumber}}}</div>", { errorNumber: errorinfo.ERROR });
